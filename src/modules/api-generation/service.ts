...
class ResourceCleanupService {
   ...
   */
  private async cleanupVercelDeployment(
    deployUrl: string,
   ...
   */
  private async cleanupGitHubRepository(
    codeUrl: string,
  ): Promise<CleanupResult> {
    try {
      ...
      console.log(
        `Would cleanup GitHub repository: ${repoInfo.owner}/${repoInfo.repo}`,
      );
      return { success: true };
    } catch (error) {
      const errorMessage =
  ...
  async cleanupProjectResources(
    ...
    if (project.deploy_url) {
      ...
      const deployResult = await this.cleanupVercelDeployment(
        project.deploy_url,
      );
  ...
  async deleteProject(
    ...
    try {
      ...
      const cleanupResults =
        await this.cleanupService.cleanupProjectResources(project);

      // Check if any cleanup operations failed
      const failedCleanups = cleanupResults.filter((result) => !result.success);
      if (failedCleanups.length > 0) {
        const errorMessages = failedCleanups
          .map((result) => result.error)
          .join(", ");
        console.error(
          `Resource cleanup failed for project ${projectId}: ${errorMessages}`,
        );
      ...
      console.log(
        `Resource cleanup completed successfully for project ${projectId}`,
      );
      ...
      if (deleteError) {
        console.error(
          `Database deletion failed for project ${projectId}:`,
          deleteError,
        );
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
