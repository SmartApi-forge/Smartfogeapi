...
export class CodeModificationService {
   ...
   */
  static async create(input: CreateCodeModification): Promise<CodeModification> {
   ...
   */
  static async getByProject(params: GetCodeModificationsByProject): Promise<CodeModification[]> {
    let query = supabase
      .from('code_modifications')
      .select('*')
   ...
   */
  static async update(input: UpdateCodeModification): Promise<CodeModification> {
   ...
   */
  static async apply(id: string): Promise<{ modification: CodeModification; updated: boolean }> {
    ...
    if (fragments && fragments.length > 0) {
      ...
      try {
        implementationCode = typeof fragment.implementation_code === 'string'
      ...
      if (updateError) {
        ...
        throw new Error(`Failed to update API fragment: ${updateError.message}`)
      }
    }
   ...
   */
  static async applyMultiple(params: ApplyMultipleModifications): Promise<CodeModification[]> {
