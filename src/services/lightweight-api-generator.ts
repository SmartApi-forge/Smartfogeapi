...
/**
 * Generates a basic README.md for the API project
 * 
 * @param projectName - The project name
...
export function generateReadme(projectName: string, endpoints: EndpointConfig[] = []): string {
  const endpointDocs = endpoints.length > 0
    ? endpoints.map(ep => `- \`${ep.methods.join(', ')}\` ${ep.path} - ${ep.resourceName}`).join('\n')
    : '- Endpoints will be documented here';

  return `# ${projectName}

A lightweight API project generated by SmartAPIForge.
