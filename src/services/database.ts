...
export const projectService = {
  ...
  async getProjectsByStatus(userId: string, status: string): Promise<Project[]> {
    const { data, error } = await supabase
      .from('projects')
      .select('*')
      .eq('user_id', userId)
...
export const jobService = {
  ...
  async updateJob(jobId: string, userId: string, updates: Partial<Job>): Promise<Job> {
    const { data, error } = await supabase
      .from('jobs')
      .update(updates)
      .eq('id', jobId)
...
export const subscriptionService = {
  ...
  subscribeToProjectUpdates(userId: string, callback: (project: Project) => void) {
    return supabase
