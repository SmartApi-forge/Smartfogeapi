...
export const messageCreated = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("process-message", async () => {
      ...
      if (error) {
        console.error(
          `Failed to update message ${messageId} for project ${project_id || "unknown"}:`,
          error,
        );
      ...
      console.log(
        `Message ${messageId} for project ${project_id || "unknown"} processed successfully`,
      );
  ...
  async ({ event, step }) => {
    ...
    try {
      ...
      jobId = await step.run("create-job", async () => {
        ...
        if (error) {
          console.warn(
            `Failed to create job (continuing without job tracking): ${error.message}`,
          );
        ...
        repoAnalysis = await step.run("analyze-repository", async () => {
          ...
          try {
            ...
            if (
              ...
              throw new Error(
                `Repository URL contains potentially dangerous characters: ${repoUrl}`,
              );
            }
            ...
            const cloneResult = await sandbox.commands.run(
              `git clone '${escapedUrl}' /home/user/repo`,
            );
            if (cloneResult.exitCode !== 0) {
              throw new Error(
                `Failed to clone repository (exit code: ${cloneResult.exitCode}): ${cloneResult.stderr || cloneResult.stdout}`,
              );
            }
            ...
            try {
              const packageJson = await sandbox.files.read(
            ...
            const dirStructure = await sandbox.commands.run(
              `find /home/user/repo -type f -name '*.js' -o -name '*.ts' -o -name '*.json' -o -name '*.md'`,
            );
            ...
            if (sandbox && typeof sandbox.kill === "function") {
              try {
                ...
                console.error(
                  "Failed to kill sandbox in catch block:",
                  killError,
                );
              }
            }
            ...
            if (sandbox && typeof sandbox.kill === "function") {
              try {
                ...
                console.error(
                  "Repository analysis sandbox cleanup error:",
                  cleanupError,
                );
              }
            }
  ...
  "implementationCode": {
    ...
    "package.json": "{\"name\": \"api\", \"version\": \"1.0.0\", \"scripts\": {\"start\": \"node index.js\"}, \"dependencies\": {\"express\": \"^4.18.0\", \"cors\": \"^2.8.5\"}}"
          ...
          if (content) {
            ...
            if (projectId && chunkCount % 10 === 0) {
              const progress = Math.min(
                95,
                Math.round((rawOutput.length / 3000) * 100),
              );
              await streamingService.emit(projectId, {
                type: "code:chunk",
                filename: "Generating...",
        ...
        try {
          ...
          const markdownMatch = rawOutput?.match(
            /```(?:json)?\s*([\s\S]*?)\s*```/,
          );
          if (markdownMatch) {
            ...
            for (const [filename, content] of Object.entries(files)) {
              ...
              for (let i = 0; i < fileContent.length; i += chunkSize) {
                ...
                const progress = Math.round(
                  ((i + chunkSize) / fileContent.length) * 100,
                );
          ...
          await MessageService.saveResult({
            content:
              errorResult.state.data.summary ||
              `Failed to parse AI result: ${error}`,
            role: "assistant",
            type: "error",
            project_id: projectId, // Add project_id for error messages too
          });
      ...
      const validationResult = await step.run(
        ...
        async () => {
          ...
          try {
            ...
            console.log(
              "🏗️ Creating E2B sandbox with timeout configuration...",
            );
            sandbox = await Sandbox.create("smart-forge-api-sandbox", {
            ...
            const openApiSpec =
              apiResult.state.data.files["openapi.yaml"] ||
              apiResult.state.data.files["openapi.json"] ||
              "";
            ...
            console.log(
              ...
              Object.keys(implementationFiles),
            );
            ...
            for (const [filename, content] of Object.entries(
              ...
              if (typeof content === "string") {
                console.log(
                  `📝 Writing file: ${filename} (${content.length} chars)`,
                );
                await sandbox.files.write(
                  `/home/user/src/${filename}`,
                  content,
                );
                ...
                if (filename === "package.json") {
                  ...
                  console.log(
                    `📝 Copying ${filename} to root directory for npm start`,
                  );
                  await sandbox.files.write(`/home/user/${filename}`, content);
                }
              } else {
                console.log(
                  `⚠️ Skipping non-string content for ${filename}:`,
                  typeof content,
                );
              }
            }
            ...
            const fileList = await sandbox.commands.run(
              'cd /home/user && find . -type f -name "*" | head -20 2>/dev/null || echo "find_failed"',
            );
            ...
            try {
              ...
              const specExists = await sandbox.commands.run(
                'cd /home/user && test -f openapi.json && echo "exists" || echo "missing"',
              );
              if (specExists.stdout?.includes("exists")) {
                // Validate JSON syntax and basic OpenAPI structure
                const validation = await sandbox.commands
                  .run(`cd /home/user && node -e "
              try {
                const fs = require('fs');
            ...
            "`);
                ...
                specValidation = {
                  exitCode: 1,
                  stdout: "",
                  stderr: "OpenAPI spec file not found",
                };
              }
            } catch (error) {
              specValidation = {
                exitCode: 1,
                stdout: "",
                stderr: `OpenAPI validation error: ${error}`,
              };
            }
            ...
            try {
              ...
              const tsFiles = await sandbox.commands.run(
                'cd /home/user/src && find . -name "*.ts" | head -1 2>/dev/null || echo "no_ts_files"',
              );
              if (tsFiles.stdout?.trim()) {
                // Ensure tsconfig.json exists in the src directory
                const tsconfigExists = await sandbox.commands.run(
                  'cd /home/user/src && test -f tsconfig.json && echo "exists" || echo "missing"',
                );
                if (tsconfigExists.stdout?.includes("missing")) {
                  // Create a basic tsconfig.json for the project
                  await sandbox.files.write(
                    "/home/user/src/tsconfig.json",
                    JSON.stringify(
                      {
                        compilerOptions: {
                          target: "es2020",
                ...
                await sandbox.commands.run(
                  "cd /home/user && npm install typescript --save-dev --silent",
                  { timeoutMs: 60000 },
                );

                // Run TypeScript compilation check
                tsValidation = await sandbox.commands.run(
                  'cd /home/user/src && npx tsc --noEmit --skipLibCheck --allowJs 2>/dev/null || echo "tsc_failed"',
                );
                tsValid = tsValidation.exitCode === 0;
                console.log(
                  "📝 TypeScript validation result:",
                  tsValid ? "✅ Valid" : "❌ Invalid",
                );
                if (!tsValid) {
                  console.log(
                ...
                const jsFiles = await sandbox.commands.run(
                  'cd /home/user/src && find . -name "*.js" | head -1 2>/dev/null || echo "no_js_files"',
                );
                if (jsFiles.stdout?.trim()) {
                  tsValidation = {
                    exitCode: 0,
                    stdout:
                      "No TypeScript files found, JavaScript files present",
                    stderr: "",
                  };
                  tsValid = true;
                } else {
                  tsValidation = {
                    exitCode: 1,
                    stdout: "",
                    stderr: "No TypeScript or JavaScript files found",
                  };
                }
              }
            } catch (error) {
              tsValidation = {
                exitCode: 1,
                stdout: "",
                stderr: `TypeScript validation error: ${error}`,
              };
            }
            ...
            try {
              ...
              const jsFiles = await sandbox.commands.run(
                'cd /home/user/src && find . -name "*.js" -type f 2>/dev/null || echo "no_js_files"',
              );
              if (jsFiles.stdout?.trim()) {
                // Check each JS file individually for better error reporting
                const fileList = jsFiles.stdout
                  .trim()
                  .split("\n")
                  .filter((f) => f.trim());
                let allValid = true;
                ...
                for (const file of fileList) {
                  const fileCheck = await sandbox.commands.run(
                    `cd /home/user/src && node --check "${file.trim()}" 2>/dev/null || echo "check_failed"`,
                  );
                ...
                syntaxValidation = {
                  exitCode: 0,
                  stdout: "No JavaScript files to validate",
                  stderr: "",
                };
                syntaxValid = true;
              }
              ...
              syntaxValidation = {
                exitCode: 1,
                stdout: "",
                stderr: `Syntax validation error: ${error}`,
              };
            }
            ...
            try {
              ...
              const packageExists = await sandbox.commands.run(
                'cd /home/user && test -f package.json && echo "exists" || echo "missing"',
              );
              if (packageExists.stdout?.includes("exists")) {
                // Check if test script exists and what it contains
                const testScriptCheck = await sandbox.commands.run(
                  "cd /home/user && node -e \"const pkg = JSON.parse(require('fs').readFileSync('package.json', 'utf8')); console.log(pkg.scripts && pkg.scripts.test ? pkg.scripts.test : 'no-test')\"; 2>/dev/null || echo \"no-test\"",
                );
                ...
                if (testScriptCheck.stdout?.includes("no-test")) {
                  ...
                  testScriptCheck.stdout?.includes("Error: no test specified")
                ) {
                  // This is the default npm test script that just echoes an error - treat as valid
                  testValidation = {
                    exitCode: 0,
                    stdout: "Default npm test script (no tests specified)",
                    stderr: "",
                  };
                  testsValid = true;
                } else {
                  ...
                  try {
                    ...
                    await sandbox.commands.run(
                      'cd /home/user && npm install --silent 2>/dev/null || echo "install_failed"',
                      { timeoutMs: 120000 },
                    ); // 2 minutes timeout
                    testValidation = await sandbox.commands.run(
                      'cd /home/user && timeout 30s npm test 2>&1 || echo "Test execution completed"',
                      { timeoutMs: 45000 },
                    );

                    // Consider tests valid if they run without crashing (even if they fail)
                    testsValid =
                      !testValidation.stdout?.includes("Test timeout") &&
                      !testValidation.stdout?.includes("command not found") &&
                      !testValidation.stdout?.includes("Cannot find module");
                  } catch (testError) {
                    testValidation = {
                      exitCode: 0,
                      stdout: "Test execution attempted but encountered issues",
                      stderr: String(testError),
                    };
                    testsValid = true; // Don't fail the entire validation for test issues
                    console.log(
                      "🧪 Test validation result:",
                      testsValid ? "✅ Valid" : "❌ Invalid",
                    );
                    if (!testsValid) {
              ...
              testValidation = {
                exitCode: 1,
                stdout: "",
                stderr: `Test validation error: ${error}`,
              };
            }
            ...
            if (specValid && tsValid) {
              ...
              try {
                ...
                const installResult = await sandbox.commands.run(
                  'cd /home/user && npm install --verbose 2>/dev/null || echo "install_failed"',
                  { timeoutMs: 120000 },
                ); // 2 minutes timeout
                console.log(
                  "Install output:",
                ...
                const portCheck = await sandbox.commands.run(
                  'cd /home/user && grep -o "PORT=[0-9]*" package.json 2>/dev/null || grep -o "port.*[0-9]*" package.json 2>/dev/null || echo "port:3000"',
                );
                ...
                await sandbox.commands.run(
                  'cp /tmp/compile_page.sh /home/user/ 2>/dev/null || echo "compile script not found"',
                );

                // First try to start from src directory, then fallback to root
                // Use proper environment variables for networking
                let startResult = await sandbox.commands.run(
                  'cd /home/user/src && test -f package.json && (HOST=0.0.0.0 PORT=3000 npm start > ../server.log 2>&1 & echo $! > ../server.pid 2>/dev/null || echo $!) || echo "start_failed"',
                  { timeoutMs: 35000 },
                );
                ...
                if (
                  ...
                  startResult.stdout.includes("start_failed")
                ) {
                  console.log("🔄 Retrying npm start from root directory...");
                  startResult = await sandbox.commands.run(
                    'cd /home/user && (HOST=0.0.0.0 PORT=3000 npm start > server.log 2>&1 & echo $! > server.pid 2>/dev/null || echo $!) || echo "start_failed"',
                    { timeoutMs: 35000 },
                  );
                }
                console.log(
                  "Server start command result:",
                  startResult.stdout,
                  startResult.stderr,
                );
                ...
                try {
                  serverWaitResult = await sandbox.commands.run(
                    'cd /home/user && test -f compile_page.sh && source compile_page.sh && wait_for_server 3000 20 || echo "wait_function_not_available"',
                  );
                  console.log("Server wait result:", serverWaitResult.stdout);
                } catch (error) {
                  console.log(
                    "Wait function not available, using fallback delay",
                  );
                  await new Promise((resolve) => setTimeout(resolve, 8000));
                }

                // Check server logs for startup confirmation
                const serverLogs = await sandbox.commands.run(
                  'cd /home/user && tail -30 server.log 2>/dev/null || echo "No server logs found"',
                );
                ...
                const pidCheck = await sandbox.commands.run(
                  'cd /home/user && (test -f server.pid && kill -0 $(cat server.pid) 2>/dev/null && echo "process running") || echo "process not found"',
                );
                ...
                const processCheck = await sandbox.commands.run(
                  'ps aux | grep -E "node.*index\.js|npm.*start" | grep -v grep 2>/dev/null || echo "no node process"',
                );
                ...
                let listeningCheck;
                try {
                  listeningCheck = await sandbox.commands.run(
                    'cd /home/user && test -f compile_page.sh && source compile_page.sh && detect_listening_ports || echo "detect_function_not_available"',
                  );
                  if (
                    listeningCheck.stdout.includes(
                      "detect_function_not_available",
                    )
                  ) {
                    // Fallback to manual detection
                    listeningCheck = await sandbox.commands.run(
                      'netstat -tlnp 2>/dev/null | grep -E ":3000|:8000|:5000" | head -3 || ss -tlnp 2>/dev/null | grep -E ":3000|:8000|:5000" | head -3 || echo "no listening ports"',
                    );
                  }
                } catch (error) {
                  listeningCheck = await sandbox.commands.run(
                ...
                console.log(
                  "Listening processes check:",
                  listeningCheck.stdout,
                );
                ...
                // Method 1: netstat
                try {
                  listeningPorts = await sandbox.commands.run(
                    'netstat -tln 2>/dev/null | grep LISTEN | grep -o ":[0-9]*" | cut -d: -f2 | sort -u || echo "no ports"',
                  );
                  if (
                    listeningPorts.stdout &&
                    !listeningPorts.stdout.includes("no ports")
                  ) {
                    const ports = listeningPorts.stdout
                      .split("\n")
                      .filter((p) => p.trim() && !isNaN(parseInt(p.trim())))
                ...
                // Method 2: ss
                try {
                  const ssResult = await sandbox.commands.run(
                    'ss -tln 2>/dev/null | grep LISTEN | grep -o ":[0-9]*" | cut -d: -f2 | sort -u || echo "no ports"',
                  );
                  if (
                    ssResult.stdout &&
                    !ssResult.stdout.includes("no ports")
                  ) {
                    const ports = ssResult.stdout
                      .split("\n")
                      .filter((p) => p.trim() && !isNaN(parseInt(p.trim())))
                      .map((p) => parseInt(p.trim()));
                    discoveredPorts.push(...ports);
                ...
                try {
                  const lsofResult = await sandbox.commands.run(
                    'lsof -i -P -n 2>/dev/null | grep LISTEN | grep -o ":[0-9]*" | cut -d: -f2 | sort -u || echo "no ports"',
                  );
                  if (
                    lsofResult.stdout &&
                    !lsofResult.stdout.includes("no ports")
                  ) {
                    const ports = lsofResult.stdout
                      .split("\n")
                      .filter((p) => p.trim() && !isNaN(parseInt(p.trim())))
                      .map((p) => parseInt(p.trim()));
                    discoveredPorts.push(...ports);
                ...
                if (discoveredPorts.length > 0) {
                  allPorts = [...discoveredPorts.slice(0, 5), ...ports].slice(
                    0,
                    8,
                  ); // Prioritize discovered ports
                ...
                try {
                  const directTest = await sandbox.commands.run(
                    `curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/health 2>/dev/null || echo "curl_failed"`,
                  );
                  console.log(
                    "Direct port 3000 test result:",
                    directTest.stdout,
                  );
                  if (
                    directTest.exitCode === 0 &&
                    !directTest.stdout.includes("curl_failed")
                  ) {
                ...
                for (const port of allPorts) {
                  try {
                    ...
                    try {
                      portCheck1 = await sandbox.commands.run(
                        `netstat -tln | grep :${port} || echo "not found"`,
                      );
                      isListening = !portCheck1.stdout.includes("not found");
                    } catch (error) {
                      console.log(
                        `netstat check failed for port ${port}:`,
                        error,
                      );
                      portCheck1 = {
                        stdout: "not found",
                    ...
                    if (!isListening) {
                      try {
                        portCheck2 = await sandbox.commands.run(
                          `ss -tln | grep :${port} || echo "not found"`,
                        );
                    ...
                    if (!isListening) {
                      try {
                        const portCheck3 = await sandbox.commands.run(
                          `lsof -i :${port} 2>/dev/null || echo "not found"`,
                        );
                        isListening = !portCheck3.stdout.includes("not found");
                      } catch (error) {
                        console.log(
                          `lsof check failed for port ${port}:`,
                          error,
                        );
                      }
                    }
                    ...
                    if (isListening) {
                      ...
                      try {
                        endpointTestResult = await sandbox.commands.run(
                          `cd /home/user && test -f compile_page.sh && source compile_page.sh && test_api_endpoint ${port} /health GET || echo "test_function_not_available"`,
                        );

                        if (
                          !endpointTestResult.stdout.includes(
                            "test_function_not_available",
                          ) &&
                          endpointTestResult.stdout.includes("✅")
                        ) {
                          healthCheckPassed = true;
                          workingPort = port;
                          healthCheckDetails.push({
                            port,
                            endpoint: "/health",
                            success: true,
                            response: endpointTestResult.stdout.substring(
                              0,
                              200,
                            ),
                          });
                          ...
                          const rootTestResult = await sandbox.commands.run(
                            `cd /home/user && source compile_page.sh && test_api_endpoint ${port} / GET`,
                          );
                          if (rootTestResult.stdout.includes("✅")) {
                            ...
                            healthCheckDetails.push({
                              ...
                              endpoint: "/",
                              success: true,
                              response: rootTestResult.stdout.substring(0, 200),
                            });
                          } else {
                            healthCheckDetails.push({
                              port,
                              endpoint: "/health,/",
                              success: false,
                              response: `Both endpoints failed: ${endpointTestResult.stdout} | ${rootTestResult.stdout}`,
                            });
                          }
                        } else {
                          ...
                          for (let retry = 0; retry < 3; retry++) {
                            try {
                              healthCheck = await sandbox.commands.run(
                                `curl -f -m 10 -s http://localhost:${port}/health 2>/dev/null || echo "curl_failed"`,
                                { timeoutMs: 15000 },
                              );
                              if (
                                healthCheck.exitCode === 0 &&
                                healthCheck.stdout &&
                                !healthCheck.stdout.includes("curl_failed")
                              ) {
                                healthCheckPassed = true;
                                workingPort = port;
                                healthCheckDetails.push({
                                  ...
                                  endpoint: "/health",
                                  success: true,
                                  response: healthCheck.stdout.substring(
                                    0,
                                    200,
                                  ),
                                });
                              ...
                              console.log(
                                `Health check retry ${retry + 1} failed for port ${port}:`,
                                error,
                              );
                              healthCheckDetails.push({
                                port,
                                endpoint: "/health",
                                success: false,
                                response: `Retry ${retry + 1} failed: ${error}`,
                              });
                            }
                          ...
                          if (!healthCheckPassed) {
                            try {
                              const rootCheck = await sandbox.commands.run(
                                `curl -f -m 10 -s http://localhost:${port}/ 2>/dev/null || echo "curl_failed"`,
                                { timeoutMs: 15000 },
                              );
                              if (
                                rootCheck.exitCode === 0 &&
                                !rootCheck.stdout.includes("curl_failed")
                              ) {
                                ...
                                healthCheckDetails.push({
                                  ...
                                  endpoint: "/",
                                  success: true,
                                  response: rootCheck.stdout.substring(0, 200),
                                });
                              } else {
                                healthCheckDetails.push({
                                  port,
                        ...
                        console.log(
                          `Endpoint test failed for port ${port}:`,
                          error,
                        );
                        healthCheckDetails.push({
                          port,
                          endpoint: "/health",
                          success: false,
                          response: `Test function error: ${error}`,
                        });
                      }
                ...
                if (!healthCheck) {
                  healthCheck = {
                    exitCode: 1,
                    stdout: "",
                    stderr: `No working port found. Tested ports: ${allPorts.join(", ")}`,
                  };
                }
                ...
                if (!("state" in apiResult) || !apiResult.state?.data) {
                  console.log(
                    "Invalid API result structure for endpoint testing",
                  );
                } else {
                  ...
                  try {
                    ...
                    console.log(
                      "Could not parse OpenAPI spec for endpoint testing",
                    );
                  }
                  ...
                  if (workingPort && parsedSpec?.paths) {
                    ...
                    for (const path of paths) {
                      const methods = Object.keys(
                        (parsedSpec.paths as any)[path],
                      );
                      const method =
                        methods.find((m) =>
                          ["get", "post"].includes(m.toLowerCase()),
                        ) || methods[0];
                      ...
                      if (method) {
                        ...
                        try {
                          const endpointTest = await sandbox.commands.run(
                            curlCmd,
                            { timeoutMs: 15000 },
                          );
                          const success =
                            endpointTest.exitCode === 0 &&
                            !endpointTest.stdout.includes("curl_failed");
                          endpoints.push({
                            path,
                            method,
                            success,
                            response:
                              endpointTest.stdout?.substring(0, 200) ||
                              endpointTest.stderr?.substring(0, 200),
                            port: workingPort,
                          });
                        } catch (error) {
                          endpoints.push({
                ...
                executionResult = {
                  serverStarted:
                    startResult.exitCode === 0 &&
                    (processCheck.stdout.includes("node") ||
                      pidCheck.stdout.includes("process running")),
                ...
                executionResult = {
                  ...
                  healthCheckDetails: [],
                  workingPort: null,
                  serverLogs: isSandboxTerminated
                    ? "Sandbox was terminated during execution"
                    : "",
                  processInfo: {
                    error: errorMessage,
            ...
            const hasWorkingServer =
              executionResult?.serverStarted &&
              executionResult?.healthCheckPassed;
            const hasValidSpec = specValid;
            const hasValidCode = tsValid && syntaxValid;
            const sandboxTerminated =
              executionResult?.errorDetails?.sandboxTerminated || false;
            ...
            const overallValid =
              ...
              (hasWorkingServer ||
                hasValidCode ||
                (sandboxTerminated && hasValidSpec));
            ...
            return {
              ...
              specValidationOutput:
                specValidation?.stdout ||
                specValidation?.stderr ||
              ...
              syntaxValidationOutput:
                syntaxValidation?.stdout ||
                syntaxValidation?.stderr ||
                "Syntax validation completed",
              testValidationOutput:
                testValidation?.stdout ||
                testValidation?.stderr ||
              ...
              validationSummary: {
                ...
                warnings: !hasWorkingServer
                  ? sandboxTerminated
                    ? [
                        "Sandbox terminated during execution - validation incomplete",
                      ]
      ...
      if (projectId) {
        await streamingService.emit(projectId, {
          ...
          summary: validationResult.overallValid
      ...
      const savedApi = await step.run("save-api", async () => {
        try {
          ...
          const safeSummary = sanitizeText(
            apiResult.state.data.summary || "Generated API",
          );
          const requirementsArray =
            Array.isArray(apiResult.state.data.requirements) &&
            apiResult.state.data.requirements.length > 0
          ...
          if (error) {
            ...
            if (
              error.code === "23502" &&
              error.message.includes("job_id") &&
              !jobId
            ) {
              console.log("Retrying API save without job_id constraint...");
              const retryRequirementsArray =
                Array.isArray(apiResult.state.data.requirements) &&
                apiResult.state.data.requirements.length > 0
                  ? apiResult.state.data.requirements
                  : [
              ...
              const { data: retryData, error: retryError } = await supabase
                ...
                .insert({
                  ...
                  openapi_spec:
                    apiResult.state.data.files["openapi.yaml"] || "",
                  implementation_code: safeStringify(
                    apiResult.state.data.files["index.js"] || {},
                  ),
      ...
      const savedResult = await step.run(
        "save-result-to-messages",
        async () => {
          try {
            const { MessageService } = await import(
              "../modules/messages/service"
            ...
            const summary =
              apiResult.state.data.summary || "Generated API successfully";
            ...
            console.log(
              "Result saved to messages and fragments:",
              result.message.id,
              result.fragment?.id,
            );
            return result;
      ...
      const versionResult = await step.run(
      ...
      await step.run("emit-complete", async () => {
        ...
        await streamingService.emit(projectId, {
          type: "complete",
          summary:
            apiResult.state.data.summary || "API generated successfully!",
  ...
  async ({ event, step }) => {
    ...
    try {
      ...
      versionId = await step.run("create-version", async () => {
        ...
        const versionNumber =
          await VersionManager.getNextVersionNumber(projectId);
...
You MUST respond with valid JSON in this exact structure:
{
  ...
  "description": "Brief summary of changes"
        ...
        try {
          ...
          // Add OpenAPI spec to files if it exists
          if (parsed.openApiSpec) {
            newFiles["openapi.json"] = JSON.stringify(
              parsed.openApiSpec,
              null,
              2,
            );
      ...
      await step.run("emit-complete", async () => {
        ...
        await streamingService.emit(projectId, {
          type: "complete",
          summary:
            apiResult.state.data.summary || "Changes applied successfully!",
          totalFiles: filesList.length,
          versionId,
        });
      ...
      await streamingService.emit(projectId, {
        type: "error",
        message:
          (error as Error).message || "An error occurred during iteration",
        stage: "Iteration",
        versionId,
      });
