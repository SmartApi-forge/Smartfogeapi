...
import { GenerationState, StreamEventWithTimestamp } from '../src/types/streaming';
 ...
 */
export function useGenerationStream(projectId: string | undefined): UseGenerationStreamResult {
  ...
  const connect = useCallback(() => {
    ...
    console.log(`[useGenerationStream] Connecting to stream for project ${projectId}`);
          ...
          switch (streamEvent.type) {
            ...
            case 'file:generating':
              ...
              const existingFileIndex = newState.generatedFiles.findIndex(
                (f: { filename: string }) => f.filename === streamEvent.filename
              );
            ...
            case 'code:chunk':
              ...
              newState.generatedFiles = newState.generatedFiles.map((file: { filename: string; content: string; path: string; isComplete: boolean }) => {
