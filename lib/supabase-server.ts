...
export const messageOperations = {
  ...
  async update(
    id: string,
    data: Partial<Omit<Message, "id" | "created_at" | "updated_at">>,
  ) {
    const { data: message, error } = await supabaseServer
      .from("messages")
      .update(data)
  ...
  async getWithFragments(options?: {
    projectId?: string;
    limit?: number;
    offset?: number;
    includeFragment?: boolean;
...
export const fragmentOperations = {
  ...
  async bulkCreate(
    dataArray: Omit<Fragment, "id" | "created_at" | "updated_at">[],
  ): Promise<Fragment[]> {
  ...
  async update(
    id: string,
    data: Partial<Omit<Fragment, "id" | "created_at" | "updated_at">>,
  ) {
    const { data: fragment, error } = await supabaseServer
      .from("fragments")
      .update(data)
